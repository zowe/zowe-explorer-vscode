...
export class USSTree extends ZoweTreeProvider implements IZoweTree<IZoweUSSTreeNode> {
    ...
    const oldFavorite: IZoweUSSTreeNode = context.isFavorite(originalNode) ? originalNode : this.mFavorites.find((temp: ZoweUSSNode) =>
    ...
    public async addFavorite(node: IZoweUSSTreeNode) {
        ...
        if (context.isFavoriteTextorBinary(temp)) {
