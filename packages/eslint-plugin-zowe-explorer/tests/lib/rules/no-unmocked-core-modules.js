...
ruleTester.run("no-unmocked-core-modules", rule, {
  valid: [
    ...
    {
      code: "import * as fs from 'fs'; vi.mock('fs')",
      options: [{ coreModuleNames: ["fs"], filePathPattern: ".*" }],
    },
    {
    ...
    {
      code: "import path from 'path'; vi.mock('path')",
  ...
  invalid: [
    {
      ...
      errors: [{ message: "Use vi.mock('fs') to mock a node core module" }],
      options: [{ coreModuleNames: ["fs"], filePathPattern: ".*" }],
      output: "import * as fs from 'fs';\nvi.mock('fs'); // `fs` must be mocked",
    },
    {
      code: "import * as path from 'path'",
      errors: [{ message: "Use vi.mock('path') to mock a node core module" }],
      options: [{ coreModuleNames: ["path"], filePathPattern: ".*" }],
