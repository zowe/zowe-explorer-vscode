    ...
    create(context, [options]) {
        ...
        return {
            ExpressionStatement(node) {
                ...
                if (expression.type === utils.AST_NODE_TYPES.ChainExpression) {
        ...
        function isHigherPrecedenceThanUnary(node) {
            ...
            return nodePrecedence > util.OperatorPrecedence.Unary;
        }
        function isAsyncIife(node) {
            if (node.expression.type !== utils.AST_NODE_TYPES.CallExpression) {
                return false;
            }
            return (node.expression.type === utils.AST_NODE_TYPES.CallExpression &&
                (node.expression.callee.type ===
                    utils.AST_NODE_TYPES.ArrowFunctionExpression ||
                    node.expression.callee.type === utils.AST_NODE_TYPES.FunctionExpression));
        }
        function isUnhandledPromise(checker, node) {
            // First, check expressions whose resulting types may not be promise-like
            if (node.type === utils.AST_NODE_TYPES.SequenceExpression) {
            ...
            if (!options.ignoreVoid &&
                node.type === utils.AST_NODE_TYPES.UnaryExpression &&
                node.operator === 'void') {
            ...
            else if (node.type === utils.AST_NODE_TYPES.ConditionalExpression) {
                // We must be getting the promise-like value from one of the branches of the
                // ternary. Check them directly.
                return (isUnhandledPromise(checker, node.alternate) ||
                    isUnhandledPromise(checker, node.consequent));
            }
            else if (node.type === utils.AST_NODE_TYPES.MemberExpression ||
                node.type === utils.AST_NODE_TYPES.Identifier ||
                node.type === utils.AST_NODE_TYPES.NewExpression) {
            ...
            else if (node.type === utils.AST_NODE_TYPES.LogicalExpression) {
...
function isPromiseLike(checker, node) {
    ...
    for (const ty of tsutils.unionTypeParts(checker.getApparentType(type))) {
        ...
        if (then === undefined || ty.getProperty('catch') === undefined) {
            continue;
        }
...
function isPromiseCatchCallWithHandler(expression) {
    return (expression.callee.type === utils.AST_NODE_TYPES.MemberExpression &&
        expression.callee.property.type === utils.AST_NODE_TYPES.Identifier &&
