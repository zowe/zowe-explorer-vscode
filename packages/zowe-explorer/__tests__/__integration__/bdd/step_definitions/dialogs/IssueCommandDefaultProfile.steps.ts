...
Given("the default tso profile is set to true", async function () {
    await browser.executeWorkbench(async (vscode) => {
        const config = vscode.workspace.getConfiguration("zowe.commands.tso");
        await config.update("defaultProfile", true, vscode.ConfigurationTarget.Global);
    });
});
