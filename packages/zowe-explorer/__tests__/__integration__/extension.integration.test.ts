...
describe("Extension Integration Tests", () => {
    ...
    describe("Creating a Session", () => {
        it("should add a session", async () => {
            ...
            inputBoxStub1.returns(
                new utils.FilterDescriptor(
            ...
            expect(
                testTree.mSessionNodes[testTree.mSessionNodes.length - 1].label
            ).to.equal(profileNamesList[0]);
        }).timeout(TIMEOUT);
    });
    ...
    describe("Creating Data Sets and Members", () => {
        it("should create a data set when zowe.createFile is invoked", async () => {
            ...
            const response = await zowe.List.dataSet(
        ...
        it("should display an error message when creating a data set that already exists", async () => {
            ...
            await expect(
                dsActions.createFile(sessionNode, testTree)
        ...
        it("should create a member when zowe.createMember is invoked", async () => {
            ...
            expect(allMembers.apiResponse.items[0].member).to.deep.equal(
                testFileName
            );
        }).timeout(TIMEOUT);
    });
    ...
    describe("Enter Pattern", () => {
        it("should output data sets that match the user-provided pattern", async () => {
            ...
            expect(testTree.mSessionNodes[1].collapsibleState).to.equal(
                vscode.TreeItemCollapsibleState.Expanded
            );
            ...
            expect(childrenFromTree[0]).to.deep.equal(
                testTree.getTreeView().selection[0]
            );
        }).timeout(TIMEOUT);
        ...
        it("should pop up a message if the user doesn't enter a pattern", async () => {
            ...
            const showInfoStub = sandbox.spy(
                vscode.window,
        ...
        it("should work when called from a saved search", async () => {
            ...
            expect(testTree.mSessionNodes[1].pattern).to.equal(
                searchPattern.toUpperCase()
            );
            expect(testTree.mSessionNodes[1].tooltip).to.equal(
                searchPattern.toUpperCase()
            );
            expect(testTree.mSessionNodes[1].collapsibleState).to.equal(
                vscode.TreeItemCollapsibleState.Expanded
    ...
    describe("Opening a PS", () => {
        ...
        it("should display an error message when openPS is passed an invalid node", async () => {
            ...
            await expect(dsActions.openPS(node, true)).to.eventually.be.rejectedWith(
                Error
            );
    ...
    describe("Saving a File", () => {
        it("should download, change, and re-upload a PS", async () => {
            ...
            const originalData = "";
            fs.writeFileSync(
                path.join(path.join(ZOWETEMPFOLDER, children[1].label)),
                originalData
            );
        }).timeout(TIMEOUT);
        ...
        it("should download, change, and re-upload a PDS member", async () => {
            ...
            fs.writeFileSync(
                path.join(
                    ZOWETEMPFOLDER,
                    children[0].label + "(" + childrenMembers[0].label + ")"
                ),
                changedData2
            );
    ...
    describe("Updating Temp Folder", () => {
        ...
        const providedPathOne = path.join(
            __dirname,
            "..",
            "..",
            "..",
        ...
        const providedPathTwo = path.join(
            __dirname,
            "..",
            "..",
            "..",
        ...
        it("should assign default temp folder, if preference is empty", async () => {
            const expectedDefaultTemp = path.join(
                __dirname,
                "..",
                "..",
                "..",
    ...
    describe("Initializing Favorites", () => {
        ...
        it("should work when provided a valid Favorites list", async () => {
            ...
            const favoritesArray = [
                `[${profileName}]: ${pattern}.EXT.PDS`,
                `[${profileName}]: ${pattern}.EXT.PS`,
                `[${profileName}]: ${pattern}.EXT.SAMPLE.PDS`,
                `[${profileName}]: ${pattern}.EXT`,
            ];
            expect(testTree3.mFavorites.map((node) => node.label)).to.deep.equal(
                favoritesArray
            );
        }).timeout(TIMEOUT);
        ...
        it("should show an error message when provided an invalid Favorites list", async () => {
            ...
            const favorites = [
                pattern + ".EXT.PDS[profileName]{pds}",
                corruptedFavorite,
            ];
            await vscode.workspace
                .getConfiguration()
            ...
            const gotCalled = showErrorStub.calledWith(
                "Favorites file corrupted: " + corruptedFavorite
            );
            expect(gotCalled).to.equal(true);
        }).timeout(TIMEOUT);
...
describe("Extension Integration Tests - USS", () => {
    ...
    describe("TreeView", () => {
        it("should create the USS TreeView", async () => {
            ...
            const nonFavorites = ussFileProvider.mSessionNodes.filter(
                (node) => node.contextValue !== FAVORITE_CONTEXT
            );
            ...
            for (const node of allNodes) {
                ...
                expect(node).to.deep.equal(
                    ussFileProvider.getTreeView().selection[0]
                );
            }
        }).timeout(TIMEOUT);
    });
    ...
    describe("Creating a Session -USS", () => {
        it("should add a session", async () => {
            ...
            expect(
                ussTestTree.mSessionNodes[ussTestTree.mSessionNodes.length - 1].label
            ).to.equal(profileNamesList[0]);
        }).timeout(TIMEOUT);
    });
    ...
    describe("Enter USS Pattern", () => {
        it("should output path that match the user-provided path", async () => {
            ...
            expect(ussTestTree1.mSessionNodes[0].collapsibleState).to.equal(
                vscode.TreeItemCollapsibleState.Expanded
            );
        ...
        it("should pop up a message if the user doesn't enter a USS path", async () => {
            ...
            const showInfoStub2 = sandbox.spy(
                vscode.window,
                "showInformationMessage"
            );
    ...
    describe("Saving a USS File", () => {
        ...
        it("should download, change, and re-upload a file", async () => {
            const changedData =
                "File Upload Test " + Math.random().toString(36).slice(2);
