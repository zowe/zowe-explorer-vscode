...
describe("Extension Integration Tests", () => {
    ...
    describe("Success Scenarios", () => {
      describe("Sequential > Sequential", () => {
        ...
        beforeEach(async () => {
          ...
          await zowe.Upload.bufferToDataSet(sessionNode.getSession(), Buffer.from("1234"), fromDataSetName).catch(
            (err) => err
          );
        });
        afterEach(async () => {
  ...
  describe("Updating Temp Folder", () => {
    ...
    it("should assign the temp folder based on preference", async () => {
      ...
      await vscode.workspace
        .getConfiguration()
        .update("Zowe-Temp-Folder-Location", { folderPath: `${testingPath}` }, vscode.ConfigurationTarget.Global);
    ...
    it("should update temp folder on preference change", async () => {
      ...
      await vscode.workspace
        .getConfiguration()
        .update("Zowe-Temp-Folder-Location", { folderPath: `${providedPathOne}` }, vscode.ConfigurationTarget.Global);

      // change preference and test for update
      await vscode.workspace
        .getConfiguration()
        .update("Zowe-Temp-Folder-Location", { folderPath: `${providedPathTwo}` }, vscode.ConfigurationTarget.Global);
