...
function createGlobalMocks() {
    ...
    Object.defineProperty(zowe, "IssueCommand", {
        value: jest.fn(),
        configurable: true,
    });
    Object.defineProperty(zowe.IssueCommand, "issueSimple", {
        value: jest.fn(),
        configurable: true,
    });
    ...
    Object.defineProperty(zowe.GetJobs, "getJclForJob", {
        value: jest.fn(),
        configurable: true,
    });
...
describe("Jobs Actions Unit Tests - Function setPrefix", () => {
    ...
    it("Checking that the prefix is set correctly on the job", async () => {
        ...
        expect(mocked(vscode.window.showInformationMessage).mock.calls.length).toBe(
            0
        );
    });
});
...
describe("Jobs Actions Unit Tests - Function stopCommand", () => {
    ...
    it("Checking that stop command of Job Node is executed properly", async () => {
        ...
        expect(
            mocked(vscode.window.showInformationMessage).mock.calls[0][0]
        ).toEqual("Command response: fake response");
    });
...
describe("Jobs Actions Unit Tests - Function downloadSpool", () => {
    ...
    it("Checking download of Job Spool", async () => {
        ...
        mocked(vscode.window.showOpenDialog).mockResolvedValue([
...
describe("Jobs Actions Unit Tests - Function submitJcl", () => {
    ...
    it("Checking submit of active text editor content as JCL", async () => {
        ...
        mocked(zowe.ZosmfSession.createBasicZosmfSession).mockReturnValue(
            blockMocks.session
        );
...
describe("Jobs Actions Unit Tests - Function submitMember", () => {
    ...
    it("Checking Submit Job for Favourite PDS Member content", async () => {
        ...
        const blockMocks = createBlockMocks();
        mocked(Profiles.getInstance).mockReturnValue(blockMocks.profileInstance);
        const favoriteSession = new ZoweDatasetNode(
            "Favorites",
            vscode.TreeItemCollapsibleState.Collapsed,
            blockMocks.datasetSessionNode,
            null
...
describe("Jobs Actions Unit Tests - Function getSpoolContent", () => {
    function createBlockMocks() {
        ...
        const profileInstance = createInstanceOfProfile(imperativeProfile);
        const treeView = createTreeView();
        const testJobTree = createJobsTree(
            session,
            iJob,
            imperativeProfile,
            treeView
    ...
    it("Checking opening of Spool Content", async () => {
        ...
        mocked(Profiles.getInstance).mockReturnValue(blockMocks.profileInstance);
        await jobActions.getSpoolContent(
...
describe("Jobs Actions Unit Tests - Function refreshJobsServer", () => {
    ...
    it("Checking common execution of function", async () => {
        ...
        mocked(zowe.ZosmfSession.createBasicZosmfSession).mockReturnValueOnce(
            blockMocks.session
        );
