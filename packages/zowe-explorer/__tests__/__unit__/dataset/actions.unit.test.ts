...
function createGlobalMocks() {
    clipboard = {
        writeText: jest
            .fn()
            .mockImplementation((value) => (mockClipboardData = value)),
    ...
    Object.defineProperty(zowe.Upload, "bufferToDataSet", {
        value: jest.fn(),
        configurable: true,
    });
    Object.defineProperty(zowe.Upload, "pathToDataSet", {
        value: jest.fn(),
        configurable: true,
    });
    ...
    Object.defineProperty(zowe.Delete, "dataSet", {
        value: jest.fn(),
        configurable: true,
    });
    Object.defineProperty(zowe, "Create", { value: jest.fn(), configurable: true });
    Object.defineProperty(zowe.Create, "dataSet", {
        value: jest.fn(),
        configurable: true,
    });
    ...
    Object.defineProperty(sharedUtils, "concatChildNodes", {
        value: jest.fn(),
        configurable: true,
    });
    ...
    Object.defineProperty(zowe.List, "dataSet", {
        value: jest.fn(),
        configurable: true,
    });
...
describe("Dataset Actions Unit Tests - Function createMember", () => {
    function createBlockMocks() {
        ...
        const datasetSessionNode = createDatasetSessionNode(
            session,
            imperativeProfile
        );
    ...
    it("Checking of common dataset member creation", async () => {
        ...
        mocked(vscode.window.withProgress).mockImplementation(
            (progLocation, callback) => {
                return callback();
            }
        );
    ...
    it("Checking failed attempt to create dataset member", async () => {
        ...
        expect(mocked(vscode.window.showErrorMessage)).toBeCalledWith(
            "Unable to create member: test Error: test"
        );
    });
...
describe("Dataset Actions Unit Tests - Function refreshPS", () => {
    ...
    it("Checking common PS dataset refresh", async () => {
        ...
        mocked(vscode.workspace.openTextDocument).mockResolvedValueOnce({
            isDirty: true,
        } as any);
        ...
        expect(mocked(vscode.commands.executeCommand)).toBeCalledWith(
...
describe("Dataset Actions Unit Tests - Function deleteDataset", () => {
    ...
    it("Checking common PS dataset failed deletion attempt due to absence on remote", async () => {
        ...
        await expect(
            dsActions.deleteDataset(node, blockMocks.testDatasetTree)
    ...
    it("Checking Favorite PS dataset deletion", async () => {
        ...
        mocked(Profiles.getInstance).mockReturnValue(blockMocks.profileInstance);
        const parent = new ZoweDatasetNode(
            "[sestest]: HLQ.TEST.DELETE.PARENT",
            vscode.TreeItemCollapsibleState.Collapsed,
            blockMocks.datasetSessionNode,
            null
        );
    ...
    it("Checking incorrect dataset failed deletion attempt", async () => {
        ...
        await expect(
            dsActions.deleteDataset(child, blockMocks.testDatasetTree)
        ).rejects.toEqual(Error("deleteDataSet() called from invalid node."));
        expect(deleteSpy).not.toBeCalled();
    });
});
...
describe("Dataset Actions Unit Tests - Function saveFile", () => {
    ...
    it("Checking common dataset saving action when no session is defined", async () => {
        ...
        mocked(sharedUtils.concatChildNodes).mockReturnValueOnce([
            nodeWithoutSession,
        ]);
        blockMocks.profileInstance.loadNamedProfile.mockReturnValueOnce(
            blockMocks.imperativeProfile
        );
        mocked(Profiles.getInstance).mockReturnValue(blockMocks.profileInstance);
        ...
        const testDocument = createTextDocument(
            "HLQ.TEST.AFILE",
            blockMocks.datasetSessionNode
        );
    ...
    it("Checking common dataset saving failed attempt due to its absence on the side of the server", async () => {
        ...
        const dataSetSpy = jest
            .spyOn(blockMocks.mvsApi, "dataSet")
    ...
    it("Checking common dataset saving", async () => {
        ...
        mocked(zowe.List.dataSet).mockResolvedValue({
            ...
            apiResponse: {
                items: [
                    { dsname: "HLQ.TEST.AFILE" },
                    { dsname: "HLQ.TEST.AFILE(mem)" },
                ],
            },
        });
    ...
    it("Checking favorite dataset saving", async () => {
        ...
        mocked(Profiles.getInstance).mockReturnValue(blockMocks.profileInstance);
        blockMocks.profileInstance.loadNamedProfile.mockReturnValue(
            blockMocks.imperativeProfile
        );
        const mockSetEtag = jest
            .spyOn(node, "setEtag")
    ...
    it("Checking favorite PDS Member saving", async () => {
        ...
        const blockMocks = createBlockMocks();
        const node = new ZoweDatasetNode(
            `[${blockMocks.imperativeProfile.name}]: HLQ.TEST.AFILE`,
            vscode.TreeItemCollapsibleState.None,
            blockMocks.datasetSessionNode,
            null,
            undefined,
        ...
        expect(blockMocks.profileInstance.loadNamedProfile).toBeCalledWith(
            blockMocks.imperativeProfile.name
        );
    });
...
describe("Dataset Actions Unit Tests - Function showDSAttributes", () => {
    ...
    it("Checking Favorite PS dataset attributes showing", async () => {
        ...
        mocked(Profiles.getInstance).mockReturnValue(blockMocks.profileInstance);
        const node = new ZoweDatasetNode(
            "[session]: AUSER.A1557332.A996850.TEST1",
            vscode.TreeItemCollapsibleState.None,
            blockMocks.datasetSessionNode,
            null
        );
    ...
    it("Checking failed attempt of dataset attributes showing (empty response)", async () => {
        ...
        await expect(
            dsActions.showDSAttributes(node, blockMocks.testDatasetTree)
        ).rejects.toEqual(
            Error(
                "No matching data set names found for query: AUSER.A1557332.A996850.TEST1"
            )
        );
        expect(mocked(vscode.window.showErrorMessage)).toBeCalledWith(
...
describe("Dataset Actions Unit Tests - Function pasteDataSet", () => {
    ...
    it("Should throw an error if invalid clipboard data is supplied when pasting to sequential data set", async () => {
        ...
        await expect(
            dsActions.pasteDataSet(node, blockMocks.testDatasetTree)
        ).rejects.toEqual(Error("Invalid clipboard. Copy from data set first"));
    ...
    it("Should throw an error when pasting to a member that already exists", async () => {
        ...
        await expect(
            ...
            Error(
                "HLQ.TEST.TO.NODE(mem1) already exists. You cannot replace a member"
            )
        );
        expect(copySpy).not.toBeCalled();
    });
    it("Should call zowe.Copy.dataSet when pasting to a favorited partitioned data set", async () => {
        ...
        mocked(vscode.window.showInputBox).mockResolvedValueOnce("mem1");
        mocked(blockMocks.testDatasetTree.findNonFavoritedNode).mockReturnValueOnce(
            nonFavoritedNode
        );
        clipboard.writeText(
            JSON.stringify({
        ...
        expect(
            mocked(blockMocks.testDatasetTree.findNonFavoritedNode)
        ).toHaveBeenCalledWith(favoritedNode);
        expect(
            mocked(blockMocks.testDatasetTree.refreshElement)
        ).toHaveBeenLastCalledWith(nonFavoritedNode);
    });
});
...
describe("Dataset Actions Unit Tests - Function createFile", () => {
    ...
    it("Checking of proper configuration being picked up for different DS types", async () => {
        ...
        const getChildrenSpy = jest.spyOn(
            blockMocks.datasetSessionNode,
            "getChildren"
        );
        ...
        mocked(vscode.window.showQuickPick).mockResolvedValueOnce(
            "Data Set Binary" as any
        );
        await dsActions.createFile(
            blockMocks.datasetSessionNode,
            blockMocks.testDatasetTree
        );
        expect(mocked(vscode.workspace.getConfiguration)).lastCalledWith(
    ...
    it("Checking of proper configuration being picked up for different DS types with credentials prompt", async () => {
        ...
        blockMocks.profileInstance.promptCredentials.mockReturnValue([
            "fake",
            "fake",
            "fake",
        ]);
    ...
    it("Checking of history being properly updated for new query", async () => {
        ...
        blockMocks.testDatasetTree.createFilterString.mockResolvedValue(
            "NODE1,NODE.*"
        );
        ...
        expect(blockMocks.testDatasetTree.addSearchHistory).toHaveBeenCalledWith(
            "NODE1,NODE.*"
        );
    });
...
describe("Dataset Actions Unit Tests - Function openPS", () => {
    ...
    it("Checking of opening for PDS Member", async () => {
        ...
        expect(mocked(fs.existsSync)).toBeCalledWith(
            path.join(
                globals.DS_DIR,
                child.getSessionNode().label.trim(),
