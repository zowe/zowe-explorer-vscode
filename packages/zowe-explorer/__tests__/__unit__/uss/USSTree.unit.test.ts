...
async function createGlobalMocks() {
    ...
    Object.defineProperty(vscode.commands, "executeCommand", {
        value: globalMocks.executeCommand,
        configurable: true,
    });
    ...
    Object.defineProperty(vscode.window, "showQuickPick", {
        value: globalMocks.showQuickPick,
        configurable: true,
    });
    ...
    Object.defineProperty(zowe, "ZosmfSession", {
        value: globalMocks.ZosmfSession,
        configurable: true,
    });
    Object.defineProperty(globalMocks.filters, "getFilters", {
        value: globalMocks.getFilters,
        configurable: true,
    });
    Object.defineProperty(vscode.window, "createQuickPick", {
        value: globalMocks.createQuickPick,
        configurable: true,
    });
    Object.defineProperty(zowe, "Utilities", {
        value: globalMocks.Utilities,
        configurable: true,
    });
    ...
    Object.defineProperty(vscode.window, "showInputBox", {
        value: globalMocks.showInputBox,
        configurable: true,
    });
    ...
    globalMocks.getFilters.mockReturnValue([
        "/u/aDir{directory}",
        "/u/myFile.txt{textFile}",
    ]);
    ...
    globalMocks.testUSSNode = createUSSNode(
        globalMocks.testSession,
        globalMocks.testProfile
    );
    globalMocks.testTree = new USSTree();
    const ussSessionTestNode = createUSSSessionNode(
        globalMocks.testSession,
        globalMocks.testProfile
    );
...
describe("USSTree Unit Tests - Function USSTree.initialize()", () => {
    it("Tests that initialize() is executed successfully", async () => {
        ...
        expectedUSSFavorites.forEach(
            (node) => (node.contextValue += globals.FAV_SUFFIX)
        );
...
describe("USSTree Unit Tests - Function USSTree.rename()", () => {
    ...
    it("Tests that USSTree.rename() is executed successfully for a favorited USS file", async () => {
        ...
        const ussFavNode = createFavoriteUSSNode(
            globalMocks.testSession,
            globalMocks.testProfile
        );
...
describe("USSTree Unit Tests - Functions USSTree.removeFileHistory()", () => {
    it("Tests that removeFileHistory() is executed successfully", async () => {
        ...
        expect(
            globalMocks.testTree.getFileHistory().includes("TESTHISTORY")
        ).toEqual(false);
    });
});
...
describe("USSTree Unit Tests - Functions USSTree.addFavorite()", () => {
    ...
    it("Tests that addFavorite() works for directories", async () => {
        ...
        expect(globalMocks.testTree.mFavorites[0].fullPath).toEqual(
            blockMocks.parentDir.fullPath
        );
    });
...
describe("USSTree Unit Tests - Function USSTree.removeFavorite()", () => {
    ...
    it("Tests that removeFavorite() works properly", async () => {
        ...
        expect(globalMocks.testTree.mFavorites[0].fullPath).toEqual(
            blockMocks.testDir.fullPath
        );

        await globalMocks.testTree.removeFavorite(
            globalMocks.testTree.mFavorites[0]
        );
...
describe("USSTree Unit Tests - Function USSTree.openItemFromPath()", () => {
    it("Tests that openItemFromPath opens a USS file in the tree", async () => {
        ...
        spyOn(globalMocks.testTree, "getChildren").and.returnValue(
            Promise.resolve([file])
        );

        await globalMocks.testTree.openItemFromPath(
            "/a/b/c.txt",
            globalMocks.testTree.mSessionNodes[1]
        );
        expect(
            globalMocks.testTree.getSearchHistory().includes("[sestest]: /a/b/c.txt")
        ).toBe(true);
    });
...
describe("USSTree Unit Tests - Function USSTree.addSession()", () => {
    it("Tests if addSession works properly", async () => {
        ...
        const foundNode = globalMocks.testTree.mSessionNodes.includes(
            testSessionNode
        );
...
describe("USSTree Unit Tests - Function USSTree.deleteSession()", () => {
    ...
    it("Tests that deleteSession works properly", async () => {
        ...
        blockMocks.testTree2.deleteSession(
            blockMocks.testTree2.mSessionNodes[blockMocks.startLength - 1]
        );
        expect(blockMocks.testTree2.mSessionNodes.length).toEqual(
            blockMocks.startLength - 1
        );
    });
});
...
describe("USSTree Unit Tests - Function USSTree.filterPrompt()", () => {
    ...
    it("Tests that filter() works properly when user enters path", async () => {
        ...
        await globalMocks.testTree.filterPrompt(
            globalMocks.testTree.mSessionNodes[1]
        );
    ...
    it("Tests that filter() exits when user cancels out of input field", async () => {
        ...
        expect(globalMocks.showInformationMessage.mock.calls[0][0]).toBe(
    ...
    it("Tests that filter() exits when user cancels the input path box (Theia)", async () => {
        ...
        blockMocks.theia = true;
        globalMocks.showQuickPick.mockReturnValueOnce(
            "\uFF0B " + "Create a new filter"
        );
        globalMocks.showInputBox.mockReturnValueOnce(undefined);
    ...
    it("Tests that filter() works with a file (Theia)", async () => {
        ...
        globalMocks.showQuickPick.mockReturnValueOnce(
            new utils.FilterDescriptor("/u/thisFile")
        );
...
describe("USSTree Unit Tests - Function USSTree.deleteSession()", () => {
    it("Tests that getChildren() returns valid list of elements", async () => {
        ...
        expect(JSON.stringify(sessNode[0].iconPath)).toContain(
            "folder-root-favorite-star-closed.svg"
        );
    });
    ...
    it("Testing that getChildren() returns correct ZoweUSSNodes when passed element of type ZoweUSSNode<session>", async () => {
        ...
        const sessChildren = await globalMocks.testTree.getChildren(
            globalMocks.testTree.mSessionNodes[1]
        );
    ...
    it("Testing that getChildren() returns correct ZoweUSSNodes when passed element of type ZoweUSSNode<favorite>", async () => {
        ...
        const favChildren = await globalMocks.testTree.getChildren(
            globalMocks.testTree.mSessionNodes[0]
        );
        ...
        Object.defineProperty(Profiles, "getInstance", {
            value: jest.fn(() => {
                return {
                    ...
                    profilesForValidation: [
                        { status: "inactive", name: globalMocks.testProfile.name },
                    ],
                    validateProfiles: jest.fn(),
                };
            }),
