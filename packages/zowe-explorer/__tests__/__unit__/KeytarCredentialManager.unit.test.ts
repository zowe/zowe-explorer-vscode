...
describe("KeytarCredentialManager Unit Tests", () => {
    ...
    KeytarCredentialManager.keytar = {
        getPassword: jest.fn(
            async (service: string, account: string): Promise<string> => {
        ...
        deletePassword: jest.fn(
            async (service: string, account: string): Promise<boolean> => {
                const isManaged: boolean = credentialStore.hasOwnProperty(
                    `${service}_${account}`
                );
                if (isManaged) {
    ...
    it("Test loading passwords from credential store", async () => {
        ...
        expect(KeytarCredentialManager.keytar.getPassword).toHaveBeenLastCalledWith(
            "Zowe-Plugin",
            "user1"
        );
    ...
    it("Test deleting passwords from credential store", async () => {
        ...
        expect(KeytarCredentialManager.keytar.deletePassword).toHaveBeenCalledTimes(
            5
        );
    ...
    it("Test saving passwords to credential store", async () => {
        ...
        expect(KeytarCredentialManager.keytar.setPassword).toHaveBeenLastCalledWith(
            "Awesome-Service",
            "user1",
            b64Encode("cupcake")
        );
