...
import { SharedActions, SharedUtils } from "../../../../src/trees/shared";
import { ZoweLocalStorage, ZoweLogger } from "../../../../src/tools";
import { USSActions, USSAtributeView, USSFileStructure, ZoweUSSNode } from "../../../../src/trees/uss";
...
describe("USS Action Unit Tests - Function saveUSSFile", () => {
    async function createBlockMocks(globalMocks) {
        const newMocks = {
            ...
            testDoc: createTextDocument(path.join(Constants.USS_DIR, "usstest", "u", "myuser", "testFile")),
            ussNode: createUSSNode(globalMocks.testSession, createIProfile()),
    ...
    it("To check Compare Function is getting triggered from Favorites", async () => {
        ...
        (testDocument as any).fileName = path.join(Constants.USS_DIR, "usstest/user/usstest/HLQ.TEST.AFILE/MEM");

        await USSActions.saveUSSFile(testDocument, blockMocks.testUSSTree);
    ...
    it("Testing that saveUSSFile is executed successfully", async () => {
        ...
        await USSActions.saveUSSFile(blockMocks.testDoc, blockMocks.testUSSTree);
    ...
    it("Tests that saveUSSFile fails when session cannot be located", async () => {
        ...
        (testDocument as any).fileName = path.join(Constants.USS_DIR, testDocument.fileName);
...
describe("USS Action Unit Tests - copy file / directory", () => {
    ...
    it("Has the proper responses for toSameSession in UssFileUtils", async () => {
        ...
        let isSameSession = USSFileStructure.toSameSession(
            {
        ...
        isSameSession = USSFileStructure.toSameSession(
            {
                localPath: "C:/some/local/path",
                ussPath: "/z/SOMEUSER/path",
                baseName: "<ROOT>",
