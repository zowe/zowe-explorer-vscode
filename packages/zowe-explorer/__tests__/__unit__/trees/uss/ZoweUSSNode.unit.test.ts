...
import { USSUtils, ZoweUSSNode, USSFileStructure } from "../../../../src/trees/uss";
...
describe("ZoweUSSNode Unit Tests - Function node.openUSS()", () => {
    ...
    it("Tests that node.openUSS() is executed successfully", async () => {
        ...
        expect(globalMocks.existsSync.mock.calls[0][0]).toBe(path.join(Constants.USS_DIR, node.getProfileName(), node.fullPath));
    ...
    it("Tests that node.openUSS() fails when an error is thrown", async () => {
        ...
        const mockFileInfo = {
            name: child.label,
            path: path.join(Constants.USS_DIR, child.getProfileName(), child.fullPath),
        };
        ...
        expect(globalMocks.openTextDocument.mock.calls.length).toBe(1);
        expect(globalMocks.openTextDocument.mock.calls[0][0]).toBe(path.join(Constants.USS_DIR, child.getProfileName(), child.fullPath));
        expect(globalMocks.mockShowTextDocument.mock.calls.length).toBe(1);
        expect(globalMocks.showErrorMessage.mock.calls.length).toBe(1);
...
describe("ZoweUSSNode Unit Tests - Function node.pasteUssTree()", () => {
    ...
    it("Tests node.pasteUssTree() reads clipboard contents and uploads files successfully", async () => {
        ...
        const mockToSameSession = jest.spyOn(USSFileStructure, "toSameSession").mockReturnValue(true);
    ...
    it("paste renames duplicate files before copying when needed", async () => {
        ...
        jest.spyOn(USSFileStructure, "toSameSession").mockReturnValueOnce(true);
