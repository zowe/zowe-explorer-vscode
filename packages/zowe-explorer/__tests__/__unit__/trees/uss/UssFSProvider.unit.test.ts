    ...
    describe("copyTree", () => {
        ...
        describe("different profiles", () => {
            it("copies a file into a destination folder - collision", async () => {
                ...
                expect(blockMocks.apiFuncs.uploadFromBuffer).toHaveBeenCalledWith(
                    Buffer.from(fileData), "/folderB/file (1)", { binary: false, encoding: undefined }
                );
                lookupMock.mockRestore();
            ...
            it("copies an untagged file cross-profile using the destination profile encoding", async () => {
                ...
                const lookupMock = vi.spyOn(UssFSProvider.instance, "_lookupAsFile" as any).mockReturnValue({
                    name: "file", type: FileType.File,
                    metadata: { path: "/sestest/folderA/file", profile: blockMocks.profile },
                    wasAccessed: true, data: fileData, ctime: 0, mtime: 0, size: 0,
                    encoding: undefined,
                });
                await (UssFSProvider.instance as any).copyTree(sourceUri, destUri, {
                    overwrite: true,
                    tree: { localUri: sourceUri, ussPath: "/folderA/file", baseName: "file",
                            sessionName: "sestest", type: USSFileStructure.UssFileType.File },
                });
                expect(blockMocks.apiFuncs.uploadFromBuffer).toHaveBeenCalledWith(
                    Buffer.from(fileData), "/folderB/file", { binary: false, encoding: "1146" }
                );
                lookupMock.mockRestore();
            });
            ...
            it("copies a binary file cross-profile without encoding", async () => {
                ...
                const lookupMock = vi.spyOn(UssFSProvider.instance, "_lookupAsFile" as any).mockReturnValue({
                    name: "file.bin", type: FileType.File,
                ...
                await (UssFSProvider.instance as any).copyTree(sourceUri, destUri, {
                    overwrite: true,
                    tree: { localUri: sourceUri, ussPath: "/folderA/file.bin", baseName: "file.bin",
                            sessionName: "sestest", type: USSFileStructure.UssFileType.File },
                });
                expect(blockMocks.apiFuncs.uploadFromBuffer).toHaveBeenCalledWith(
                    Buffer.from(fileData), "/folderB/file.bin", { binary: true, encoding: undefined }
                );
                lookupMock.mockRestore();
            });
        });
