...
describe("UssFSProvider", () => {
    ...
    describe("readDirectory cache validation", () => {
        let lookupAsDirMock: jest.SpyInstance;
        let listFilesSpy: jest.SpyInstance;
        let remoteLookupSpy: jest.SpyInstance;
        let warnLoggerSpy: jest.SpyInstance;

        beforeEach(() => {
            lookupAsDirMock = jest.spyOn((UssFSProvider as any).prototype, "_lookupAsDirectory");
            listFilesSpy = jest.spyOn(UssFSProvider.instance, "listFiles");
            remoteLookupSpy = jest.spyOn(UssFSProvider.instance, "remoteLookupForResource");
        ...
        it("should trigger a remote lookup if the cached directory's mtime differs from the parent listing", async () => {
            const cachedDir = new UssDirectory("aFolder");
            cachedDir.mtime = 1000;
            cachedDir.metadata = { profile: testProfile, path: "/sestest/aFolder" };
            (cachedDir as any).allFetched = true;
            lookupAsDirMock.mockReturnValue(cachedDir);

            listFilesSpy.mockResolvedValue({
                success: true,
                apiResponse: {
                    items: [{ name: "aFolder", mtime: 2000 }],
                },
            });

            const freshDir = new UssDirectory("aFolder");
            freshDir.mtime = 2000;
            freshDir.entries.set("newFile.txt", new UssFile("newFile.txt"));
            (freshDir as any).allFetched = true;
            remoteLookupSpy.mockResolvedValue(freshDir);

            const result = await UssFSProvider.instance.readDirectory(testUris.folder);

            expect(listFilesSpy).toHaveBeenCalled();
            expect(remoteLookupSpy).toHaveBeenCalledWith(testUris.folder);
            expect(result).toContainEqual(["newFile.txt", FileType.File]);
        });

        it("should NOT trigger a remote lookup if mtime matches the parent listing (Valid Cache)", async () => {
            const mtime = 5000;
            const cachedDir = new UssDirectory("aFolder");
            cachedDir.mtime = mtime;
            cachedDir.metadata = { profile: testProfile, path: "/sestest/aFolder" };
            cachedDir.entries.set("cachedFile.txt", new UssFile("cachedFile.txt"));
            (cachedDir as any).allFetched = true;
            lookupAsDirMock.mockReturnValue(cachedDir);
            ...
            listFilesSpy.mockResolvedValue({
                success: true,
                apiResponse: {
                    items: [{ name: "aFolder", mtime: mtime }],
                },
            });
            ...
            expect(result).toContainEqual(["cachedFile.txt", FileType.File]);
        });

        it("should trigger a remote lookup if the directory is missing from the parent listing (Deleted/Moved)", async () => {
            const cachedDir = new UssDirectory("aFolder");
            cachedDir.metadata = { profile: testProfile, path: "/sestest/aFolder" };
            (cachedDir as any).allFetched = true;
            lookupAsDirMock.mockReturnValue(cachedDir);
            ...
            listFilesSpy.mockResolvedValue({
                success: true,
                apiResponse: {
                    items: [{ name: "otherFolder" }],
                },
            });

            remoteLookupSpy.mockResolvedValue(cachedDir);

            await UssFSProvider.instance.readDirectory(testUris.folder);

            expect(remoteLookupSpy).toHaveBeenCalledWith(testUris.folder);
        });

        it("should degrade gracefully (use cache) if checking the parent directory fails", async () => {
            const cachedDir = new UssDirectory("aFolder");
            cachedDir.entries.set("cachedFile.txt", new UssFile("cachedFile.txt"));
            cachedDir.metadata = { profile: testProfile, path: "/sestest/aFolder" };
            (cachedDir as any).allFetched = true;
            ...
            listFilesSpy.mockRejectedValue(new Error("Permission denied"));

            const result = await UssFSProvider.instance.readDirectory(testUris.folder);

            expect(warnLoggerSpy).toHaveBeenCalledWith(expect.stringContaining("Failed to validate mtime"));
            expect(remoteLookupSpy).not.toHaveBeenCalled();
            expect(result).toContainEqual(["cachedFile.txt", FileType.File]);
        });

        it("should skip validation and use cache if URI is Root", async () => {
            const rootDir = new UssDirectory("sestest");
            rootDir.metadata = { profile: testProfile, path: "/" };
            (rootDir as any).allFetched = true;
            lookupAsDirMock.mockReturnValue(rootDir);

            await UssFSProvider.instance.readDirectory(testUris.session);
