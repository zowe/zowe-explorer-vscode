...
describe("AttributeView unit tests", () => {
    ...
    const treeProvider = { refreshElement: vi.fn(), refresh: vi.fn() } as unknown as IZoweTree<IZoweUSSTreeNode>;
    const createDirMock = vi.spyOn(UssFSProvider.instance, "createDirectory").mockImplementation((() => undefined) as any);
    ...
    const updateAttrsApiMock = vi.fn();
    const updateAttributesMock = vi.spyOn(node, "setAttributes").mockImplementation((() => undefined) as any);
    ...
    beforeAll(() => {
        ...
        vi.spyOn(SharedContext, "isUssDirectory").mockReturnValue(false);
    ...
    it("refreshes properly when webview sends 'refresh' command", async () => {
        ...
        node.getParent = vi.fn().mockReturnValueOnce({ label: "parent node" } as IZoweUSSTreeNode);
        await (view as any).onDidReceiveMessage({ command: "refresh" });
    ...
    it("dispatches node data to webview when 'ready' command is received", async () => {
        ...
        const getAttributesMock = vi.spyOn(ZoweUSSNode.prototype, "getAttributes").mockResolvedValue(attrs as any);
    ...
    it("updates attributes when 'update-attributes' command is received", async () => {
        const getAttributesMock = vi.spyOn(ZoweUSSNode.prototype, "getAttributes");
    ...
    it("handles any errors while updating attributes", async () => {
        const getAttributesMock = vi.spyOn(ZoweUSSNode.prototype, "getAttributes").mockRejectedValue(new Error("Failed to update attributes"));
        await (view as any).onDidReceiveMessage({
            command: "update-attributes",
