...
describe("SharedUtils helpers", () => {
    ...
    test("isSamePhysicalDataset handles non-array vols and compares ignoring order", async () => {
        const srcProfile = { name: "SRC" } as any;
        const dstProfile = { name: "DST" } as any;
        const dsn = "USER.TEST.PDS";

        // src returns vols as a single string, dst returns as array out-of-order
        const srcApi = {
            dataSet: jest.fn().mockResolvedValue({
                apiResponse: { items: [{ dsname: dsn, vols: "VOL02" }] },
            }),
        };
        const dstApi = {
            dataSet: jest.fn().mockResolvedValue({
                apiResponse: { items: [{ dsname: dsn, vols: ["VOL01", "VOL02"] }] },
            }),
        };
        ...
        // different vols lengths -> should be false
        const same = await SharedUtils.isSamePhysicalDataset(srcProfile, dstProfile, dsn);
        expect(same).toBe(false);
    ...
    test("isSamePhysicalDataset returns false if API throws (safe fallback)", async () => {
        const srcProfile = { name: "SRC" } as any;
        const dstProfile = { name: "DST" } as any;
        const dsn = "SOME.DSN";

        const srcApi = { dataSet: jest.fn().mockRejectedValue(new Error("boom")) };
        const dstApi = { dataSet: jest.fn().mockResolvedValue({ apiResponse: { items: [] } }) };
    ...
    test("isLikelySameUssObjectByUris normalizes backslashes and forward slashes", async () => {
        const srcNode: any = { fullPath: "\\u\\foo\\bar" }; // backslashes
        const targetParent: any = { fullPath: "/u/foo" }; // forward slashes
        const label = "bar";

        const res = await SharedUtils.isLikelySameUssObjectByUris(srcNode, targetParent, label);
        expect(res).toBe(true);

        const notSame = await SharedUtils.isLikelySameUssObjectByUris({ fullPath: "/u/different" } as any, targetParent, label);
        expect(notSame).toBe(false);
    });

    test("hasNameCollision trims and compares case-insensitively", () => {
        const src = [" MemberA ", "memberB"];
        const dst = ["membera", "OTHER"];
        expect(SharedUtils.hasNameCollision(src, dst)).toBe(true);

        expect(SharedUtils.hasNameCollision(["one"], ["two"])).toBe(false);
    });
});