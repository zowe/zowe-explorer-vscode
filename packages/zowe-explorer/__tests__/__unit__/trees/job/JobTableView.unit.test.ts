...
describe("JobTableView unit tests", () => {
    ...
    describe("cacheChildren", () => {
        it("calls getChildren to retrieve the session node's children", async () => {
            const blockMocks = getBlockMocks();
            const getChildrenMock = vi.spyOn(blockMocks.sessionNode, "getChildren").mockResolvedValue([blockMocks.jobNode]);
    ...
    describe("cancelJobs", () => {
        ...
        it("calls JobActions.cancelJobs for each selected job", async () => {
            const blockMocks = getBlockMocks();
            const cancelJobsMock = vi.spyOn(JobActions, "cancelJobs").mockImplementation((() => undefined) as any);
            const cacheChildrenMock = vi.spyOn(JobTableView as any, "cacheChildren").mockImplementation((() => undefined) as any);
    ...
    describe("deleteJobs", () => {
        it("calls JobActions.deleteJobs for each selected job", async () => {
            const blockMocks = getBlockMocks();
            const deleteCommandMock = vi.spyOn(JobActions, "deleteCommand").mockImplementation((() => undefined) as any);
            const cacheChildrenMock = vi.spyOn(JobTableView as any, "cacheChildren").mockImplementation((() => undefined) as any);
            (JobTableView as any).cachedChildren = [blockMocks.jobNode];
            const setContentMock = vi.fn();
    ...
    describe("downloadJobs", () => {
        it("calls JobActions.downloadSpool for each selected job", async () => {
            const blockMocks = getBlockMocks();
            const downloadSpoolMock = vi.spyOn(JobActions, "downloadSpool").mockImplementation((() => undefined) as any);
            (JobTableView as any).cachedChildren = [blockMocks.jobNode];
    ...
    describe("generateTable", () => {
        ...
        it("updates an existing table if it exists", async () => {
            const blockMocks = getBlockMocks();
            const setTitleMock = vi.spyOn(Table.View.prototype, "setTitle").mockImplementation((() => undefined) as any);
            const setContentMock = vi.spyOn(Table.View.prototype, "setContent").mockImplementation((() => undefined) as any);
            await expect((JobTableView as any).generateTable({ extensionPath: "/a/b/c/" } as any, blockMocks.sessionNode)).resolves.toBeInstanceOf(
                Table.Instance
            );
            expect(setContentMock).toHaveBeenCalled();
    ...
    describe("getJcl", () => {
        it("calls JobActions.downloadJcl for the selected job", async () => {
            const blockMocks = getBlockMocks();
            const downloadJclMock = vi.spyOn(JobActions, "downloadJcl").mockImplementation((() => undefined) as any);
            (JobTableView as any).cachedChildren = [blockMocks.jobNode];
    ...
    describe("handleCommand", () => {
        it("caches children and calls generateTable", async () => {
            ...
            const generateTableMock = vi.spyOn(JobTableView as any, "generateTable").mockResolvedValue(fakeView);
            const setTableViewMock = vi.spyOn(TableViewProvider.prototype, "setTableView").mockImplementation((() => undefined) as any);
        ...
        it("returns early if invalid number of nodes are provided", async () => {
            const blockMocks = getBlockMocks();
            const getChildrenSpy = vi.spyOn(blockMocks.sessionNode, "getChildren");
            const generateTableSpy = vi.spyOn(JobTableView as any, "generateTable");
            const setTableViewSpy = vi.spyOn(TableViewProvider.prototype, "setTableView");
