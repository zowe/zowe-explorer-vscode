...
import { IZoweDatasetTreeNode, ZoweTreeNode, ZoweExplorerApiRegister } from "@zowe/zowe-explorer-api";
    ...
    try {
      if (contextually.isSessionNotFav(this)) {
        ...
        for (const pattern of this.pattern.split(",")) {
          responses.push(
            await ZoweExplorerApiRegister.getMvsApi(this.getProfile()).dataSet(pattern.trim(), { attributes: true })
          );
        }
      } else {
