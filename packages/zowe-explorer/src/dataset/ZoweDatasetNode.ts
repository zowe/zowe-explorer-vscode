        ...
        responses = await vscode.window.withProgress(
            {
                ...
                title: localize(
                    "ZoweJobNode.getJobs.progress",
                    "Get Dataset list command submitted."
                ),
            },
            () => {
        ...
        responses.forEach((response) => {
            ...
            for (const item of response.apiResponse.items) {
                const existing = this.children.find(
                    (element) => element.label.trim() === item.dsname
                );
                if (existing) {
        ...
        try {
            if (contextually.isSessionNotFav(this)) {
                ...
                for (const pattern of this.pattern.split(",")) {
                    responses.push(
                        await ZoweExplorerApiRegister.getMvsApi(
                            this.getProfile()
                        ).dataSet(pattern.trim(), { attributes: true })
                    );
                }
            } else {
                ...
                responses.push(
                    await ZoweExplorerApiRegister.getMvsApi(
                        this.getProfile()
                    ).allMembers(label, { attributes: true })
                );
            }
        } catch (err) {
            try {
