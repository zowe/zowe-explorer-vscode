...
export async function initDatasetProvider(context: vscode.ExtensionContext): Promise<DatasetTree> {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("zowe.ds.refreshNode", async (node, nodeList) => {
            ...
            for (const item of selectedNodes) {
                await dsActions.refreshPS(item as IZoweDatasetTreeNode);
            }
        })
    );
    context.subscriptions.push(
        vscode.commands.registerCommand("zowe.ds.refreshDataset", async (node, nodeList) => {
            ...
            for (const item of selectedNodes) {
                await dsActions.refreshDataset(item as IZoweDatasetTreeNode, datasetProvider);
            }
        })
    );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("zowe.ds.showAttributes", async (node, nodeList) => {
            ...
            for (const item of selectedNodes) {
                await dsActions.showAttributes(item as IZoweDatasetTreeNode, datasetProvider);
            }
        })
    );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("zowe.ds.pasteDataSets", async (node: ZoweDatasetNode) => {
            ...
            await dsActions.refreshDataset(node.getParent() as IZoweDatasetTreeNode, datasetProvider);
        })
    );
