...
import { Gui, IZoweDatasetTreeNode, IZoweTreeNode, ZosEncoding } from "@zowe/zowe-explorer-api";
...
export async function initDatasetProvider(context: vscode.ExtensionContext): Promise<DatasetTree> {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("zowe.ds.copyName", async (node: IZoweDatasetTreeNode): Promise<void> => dsActions.copyName(node))
    );
