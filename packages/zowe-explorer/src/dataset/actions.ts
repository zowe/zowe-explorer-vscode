...
export async function openPS(
  ...
  if (Profiles.getInstance().validProfile === ValidProfileEnum.VALID) {
    try {
      ...
      globals.LOG.error(
        localize("openPS.log.error.openDataSet", "Error encountered when opening data set! ") + JSON.stringify(err)
      );
...
export async function showDSAttributes(parent: IZoweDatasetTreeNode, datasetProvider: IZoweTree<IZoweDatasetTreeNode>) {
  ...
  if (Profiles.getInstance().validProfile === ValidProfileEnum.VALID) {
    ...
    try {
      ...
      globals.LOG.error(
        localize("showDSAttributes.log.error", "Error encountered when listing attributes! ") + JSON.stringify(err)
      );
      errorHandling(
        err,
...
export async function deleteDataset(node: IZoweTreeNode, datasetProvider: IZoweTree<IZoweDatasetTreeNode>) {
  ...
  try {
    ...
    globals.LOG.error(
      localize("deleteDataSet.delete.log.error", "Error encountered when deleting data set! ") + JSON.stringify(err)
    );
  ...
  if (fav) {
    datasetProvider.mSessionNodes.forEach((ses) => {
      if (
        node.label.substring(node.label.indexOf("[") + 1, node.label.indexOf("]")) === ses.label.trim() ||
        node
          .getParent()
          .getLabel()
          .substring(node.getParent().getLabel().indexOf("["), node.getParent().getLabel().indexOf("]")) === ses.label
      ) {
        ses.dirty = true;
      }
    });
...
export async function refreshPS(node: IZoweDatasetTreeNode) {
  ...
  try {
    ...
    globals.LOG.error(
      localize("refreshPS.log.error.refresh", "Error encountered when refreshing data set view: ") + JSON.stringify(err)
    );
...
export async function pasteDataSet(node: IZoweDatasetTreeNode, datasetProvider: IZoweTree<IZoweDatasetTreeNode>) {
  ...
  if (Profiles.getInstance().validProfile === ValidProfileEnum.VALID) {
    ...
    try {
      ({ dataSetName: beforeDataSetName, memberName: beforeMemberName, profileName: beforeProfileName } = JSON.parse(
...
export async function saveFile(doc: vscode.TextDocument, datasetProvider: IZoweTree<IZoweDatasetTreeNode>) {
  ...
  if (sesNode) {
    ...
    globals.LOG.debug(
      localize("saveFile.log.debug.sessionNode", "couldn't find session node, loading profile with CLI profile manager")
    );
