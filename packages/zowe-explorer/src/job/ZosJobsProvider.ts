...
export class ZosJobsProvider extends ZoweTreeProvider implements IZoweTree<IZoweJobTreeNode> {
  ...
  public async saveSearch(node: IZoweJobTreeNode) {
    ...
    const favJob = new Job(
      "[" + node.getProfileName() + "]: " + this.createSearchLabel(node.owner, node.prefix, node.searchId),
  ...
  public async updateFavorites() {
    const settings: any = {
      ...vscode.workspace.getConfiguration().get(ZosJobsProvider.persistenceSchema),
    };
    if (settings.persistence) {
      ...
      await vscode.workspace
        .getConfiguration()
        .update(ZosJobsProvider.persistenceSchema, settings, vscode.ConfigurationTarget.Global);
    }
  }
  ...
  private createJobsFavorite(node: IZoweJobTreeNode): IZoweJobTreeNode {
    const favJob = new Job(
      "[" + node.getSessionNode().label + "]: " + node.label.substring(0, node.label.lastIndexOf(")") + 1),
      vscode.TreeItemCollapsibleState.Collapsed,
      node.getParent(),
      node.getSession(),
      node.job,
