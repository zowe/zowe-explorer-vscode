...
export async function searchInAllLoadedItems(
            ...
            if (contextually.isDsMember(item)) {
                qpItem = new FilterItem(
                    `[${item
                        ...
                        .label.trim()}]: ${item
                        .getParent()
                        .label.trim()}(${item.label.trim()})`,
                    "Data Set Member"
                );
            } else {
                qpItem = new FilterItem(
                    `[${item.getSessionNode().label.trim()}]: ${item.label.trim()}`,
                    "Data Set"
                );
            }
            qpItems.push(qpItem);
    ...
    if (pattern) {
        ...
        if (pattern.indexOf("(") !== -1) {
            nodeName = pattern.substring(
                pattern.indexOf(" ") + 1,
                pattern.indexOf("(")
            );
            memberName = pattern.substring(
                pattern.indexOf("(") + 1,
                pattern.indexOf(")")
            );
        ...
        if (pattern.indexOf("/") !== -1) {
            // USS nodes
            const node = items.filter(
                (item) => item.fullPath.trim() === filePath
            )[0];
            ...
            const sessionNode = sessions.filter(
                (session) => session.label.trim() === sessionName
            )[0];
            let children = await datasetProvider.getChildren(sessionNode);
            const node = children.filter(
                (child) => child.label.trim() === nodeName
            )[0];

            if (memberName) {
                // Members
                children = await datasetProvider.getChildren(node);
                const member = children.filter(
                    (child) => child.label.trim() === memberName
                )[0];
                node.collapsibleState = vscode.TreeItemCollapsibleState.Expanded;
                await datasetProvider
                    .getTreeView()
                    .reveal(member, { select: true, focus: true, expand: false });
                ...
                await datasetProvider
                    .getTreeView()
                    .reveal(node, { select: true, focus: true, expand: false });
...
export async function openRecentMemberPrompt(
    ...
    const fileHistory = [
        ...datasetTree.getFileHistory(),
        ...ussTree.getFileHistory(),
    ];
    ...
    if (fileHistory.length > 0) {
        ...
        const items: vscode.QuickPickItem[] = fileHistory.map(
        ...
        vscode.window.showInformationMessage(
            localize("getRecentMembers.empty", "No recent members found.")
        );
        return;
    }
}
...
export async function returnIconState(node: IZoweNodeType) {
    if (
        getIconByNode(node) === getIconById(IconId.sessionActive) ||
        getIconByNode(node) === getIconById(IconId.sessionInactive)
    ) {
