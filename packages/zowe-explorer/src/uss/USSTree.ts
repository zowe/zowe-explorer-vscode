  ...
  public async updateFavorites() {
    ...
    if (settings.persistence) {
      settings.favorites = this.mFavorites.map((fav) => {
        ...
        return (
          (fav.fullPath.startsWith(correctedProfileName) ? fav.fullPath : correctedProfileName + fav.fullPath) +
          "{" +
      ...
      await vscode.workspace
        .getConfiguration()
        .update(USSTree.persistenceSchema, settings, vscode.ConfigurationTarget.Global);
    }
  }
  ...
  public async filterPrompt(node: IZoweUSSTreeNode) {
    ...
    if (Profiles.getInstance().validProfile === ValidProfileEnum.VALID) {
      if (contextually.isSessionNotFav(node)) {
        ...
        const faveNode = node;
        sessionNode = this.mSessionNodes.find((tempNode) => tempNode.getProfileName() === session);
