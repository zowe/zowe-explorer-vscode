 ...
 */
export class USSTree extends ZoweTreeProvider
    implements IZoweTree<IZoweUSSTreeNode> {
    ...
    public async getChildren(
        element?: IZoweUSSTreeNode | undefined
    ): Promise<IZoweUSSTreeNode[]> {
        if (element) {
    ...
    public async addSession(sessionName?: string, profileType?: string) {
        ...
        if (sessionName) {
            const profile: IProfileLoaded = Profiles.getInstance().loadNamedProfile(
                sessionName
            );
            if (profile) {
    ...
    public async saveSearch(node: IZoweUSSTreeNode) {
        const label =
            "[" + node.getSessionNode().getProfileName() + "]: " + node.fullPath;
    ...
    public async updateFavorites() {
        const settings: any = {
            ...vscode.workspace.getConfiguration().get(USSTree.persistenceSchema),
        };
        if (settings.persistence) {
            settings.favorites = this.mFavorites.map((fav) => {
                const correctedProfileName = "[" + fav.getProfileName() + "]: ";
                return (
                    (fav.fullPath.startsWith(correctedProfileName)
                        ? fav.fullPath
                        : correctedProfileName + fav.fullPath) +
                    "{" +
    ...
    public async filterPrompt(node: IZoweUSSTreeNode) {
        if (this.log) {
            this.log.debug(
                localize(
                    "filterPrompt.log.debug.promptUSSPath",
        ...
        if (Profiles.getInstance().validProfile === ValidProfileEnum.VALID) {
            if (contextually.isSessionNotFav(node)) {
                ...
                const options: vscode.InputBoxOptions = {
                    prompt: localize(
                        "filterPrompt.option.prompt.search",
                        "Create a new filter"
                    ),
                    value: remotepath,
                };
                ...
                if (!remotepath || remotepath.length === 0) {
                    vscode.window.showInformationMessage(
                        localize("filterPrompt.enterPath", "You must enter a path.")
                    );
                    return;
                }
            } else {
                // executing search from saved search in favorites
                remotepath = node.label
                    .trim()
                ...
                const faveNode = node;
                sessionNode = this.mSessionNodes.find(
                    (tempNode) => tempNode.getProfileName() === session
                );
                if (
                    !sessionNode.getSession().ISession.user ||
                    !sessionNode.getSession().ISession.password
                ) {
    ...
    public async initialize(log: Logger) {
        ...
        lines.forEach((line) => {
            ...
            try {
                ...
                const session = ZoweExplorerApiRegister.getUssApi(
                    profile
                ).getSession();
                ...
                if (directorySearchPattern.test(line)) {
                    ...
                    node.command = {
                        command: "zowe.uss.ZoweUSSNode.open",
                        title: localize(
                            "initializeUSSFavorites.lines.title",
                            "Open"
                        ),
                        arguments: [node],
                    };
                ...
                const errMessage: string =
                    ...
                    localize(
                        "initializeUSSFavorites.error.profile4",
                        " user settings."
                    );
    ...
    public async openItemFromPath(itemPath: string, sessionNode: IZoweUSSTreeNode) {
        ...
        sessionNode.collapsibleState = vscode.TreeItemCollapsibleState.Expanded;
        sessionNode.tooltip = sessionNode.fullPath = `/${nodePath
        ...
        this.addSearchHistory(
            `[${sessionNode.getProfileName()}]: /${nodePath.join("/")}`
        );
        await sessionNode.getChildren();

        // Reveal the searched item in the tree
        const selectedNode: IZoweUSSTreeNode = sessionNode.children.find(
            (elt) => elt.label === selectedNodeName
        );
        if (selectedNode) {
