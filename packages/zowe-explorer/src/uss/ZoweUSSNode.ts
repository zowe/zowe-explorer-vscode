...
export class ZoweUSSNode extends ZoweTreeNode implements IZoweUSSTreeNode {
    ...
    constructor(
        ...
        if (mProfileName) {
            ...
            this.setProfile(
                Profiles.getInstance().loadNamedProfile(mParent.mProfileName)
            );
        }
        this.etag = etag ? etag : "";
    ...
    public async getChildren(): Promise<IZoweUSSTreeNode[]> {
        if (
            (!this.fullPath && contextually.isSession(this)) ||
            contextually.isDocument(this)
        ) {
            return [];
        }
        ...
        if (!this.label) {
            vscode.window.showErrorMessage(
                localize("getChildren.error.invalidNode", "Invalid node")
            );
        ...
        try {
            responses.push(
                await vscode.window.withProgress(
                    ...
                    () => {
                        return ZoweExplorerApiRegister.getUssApi(
                            this.getProfile()
                        ).fileList(this.fullPath);
                    }
                )
            );
        } catch (err) {
            await errorHandling(
                err,
                this.label,
                localize("getChildren.error.response", "Retrieving response from ") +
                    `uss-file-list`
            );
        ...
        responses.forEach((response) => {
            ...
            for (const item of response.apiResponse.items) {
                const existing = this.children.find(
                    (element) => element.label.trim() === item.name
                );
                if (existing) {
                    ...
                    if (item.mode.startsWith("d")) {
                        ...
                        let temp;
                        if (
                            this.getSessionNode().binaryFiles.hasOwnProperty(
                                this.fullPath + "/" + item.name
                            )
                        ) {
    ...
    public async refreshAndReopen(hasClosedInstance = false) {
        ...
        if (
            !this.isFolder &&
            (hasClosedInstance || (this.binary && this.downloaded))
        ) {
    ...
    public async deleteUSSNode(
        ...
        try {
            ...
            vscode.window.showErrorMessage(
                localize("deleteUSSNode.error.node", "Unable to delete node: ") +
                    err.message
            );
            throw err;
        }
        ...
        ussFileProvider.removeFileHistory(
            `[${this.getProfileName()}]: ${this.parentPath}/${this.label}`
        );
            ...
            try {
                ...
                switch (true) {
                    case contextually.isFavoriteContext(this.getParent()):
                        label = this.label
                            .substring(this.label.indexOf(":") + 1)
                            .trim();
                        break;
                ...
                if (download || !fs.existsSync(documentFilePath)) {
                    ...
                    const chooseBinary =
                        ...
                        (await ZoweExplorerApiRegister.getUssApi(
                            profile
                        ).isFileTagBinOrAscii(this.fullPath));
                ...
                ussFileProvider.addFileHistory(
                    `[${this.getProfile().name}]: ${this.fullPath}`
                );
                ussFileProvider
                    .getTreeView()
                    .reveal(this, { select: true, focus: true, expand: false });
    ...
    public async refreshUSS() {
        ...
        try {
            ...
            if ((isDirty && !this.isDirtyInEditor && !wasSaved) || !isDirty) {
                ...
                const response = await ZoweExplorerApiRegister.getUssApi(
                    prof
                ).getContents(this.fullPath, {
            ...
            if (
                err.message.includes(
                    localize("refreshUSS.error.notFound", "not found")
                )
            ) {
    ...
    public async initializeFileOpening(documentPath: string, previewFile?: boolean) {
        ...
        if (!this.binary) {
            ...
            if (openingTextFailed) {
                const yesResponse = localize(
                    "openUSS.log.info.failedToOpenAsText.yes",
                    "Re-download"
                );
                const noResponse = localize(
    ...
    public getUSSDocumentFilePath() {
        return path.join(
            globals.USS_DIR || "",
            "/" + this.getSessionNode().getProfileName() + "/",
            this.fullPath
        );
    }
