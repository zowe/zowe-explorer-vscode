...
- Renamed the `Connect to zowex server on host...` command to `Deploy zowex server on host and connect...`. [#4436](https://github.com/zowe/zowe-explorer-vscode/pull/4436)
- Added an error message in the case that Zowe Explorer needs to deploy the backend SSH server `zowex`, but the user does not have write access to the specified directory when issuing the `Deploy zowex server on host and connect...` command or when validating `ssh` profiles. [#4436](https://github.com/zowe/zowe-explorer-vscode/pull/4436)
