...
describe("AbstractFtpApi", () => {
    ...
    it("should show a fatal message when trying to call getStatus with invalid credentials.", async () => {
        ...
        try {
            ...
            expect(err.message).toContain(
                "Zowe Explorer FTP Extension: Invalid Credentials. Please ensure the username and password for undefined are valid or this may lead to a lock-out."
            );
            expect(err).not.toBeUndefined();
    ...
    it("should show a different fatal message when trying to call getStatus and an exception occurs.", async () => {
        ...
        try {
            ...
            expect(ZoweLogger.getExtensionName).toHaveBeenCalled();
            expect(err.message).toContain("Zowe Explorer FTP Extension: Something happened");
            expect(err).not.toBeUndefined();
            expect(err).toBeInstanceOf(ZoweFtpExtensionError);
        }
    });
