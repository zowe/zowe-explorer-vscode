...
class ZosmfApiCommon implements ZoweExplorerApi.ICommon {
    ...
    public getSession(profile?: IProfileLoaded): Session {
        if (!this.session) {
            this.session = zowe.ZosmfSession.createBasicZosmfSession(
    ...
    public async getStatus(
        validateProfile?: IProfileLoaded,
        profileType?: string
    ): Promise<string> {
        // This API call is specific for z/OSMF profiles
        if (profileType === "zosmf") {
            const validateSession = await zowe.ZosmfSession.createBasicZosmfSession(
                validateProfile.profile
            );
            const sessionStatus = await zowe.CheckStatus.getZosmfInfo(
                validateSession
            );
...
export class ZosmfUssApi extends ZosmfApiCommon implements ZoweExplorerApi.IUss {
    ...
    public async putContent(
        ...
        options.task = task;
        return zowe.Upload.fileToUssFile(
            this.getSession(),
            inputFilePath,
            ussFilePath,
            options
    ...
    public async uploadDirectory(
        ...
        return zowe.Upload.dirToUSSDirRecursive(
            this.getSession(),
            inputDirectoryPath,
            ussDirectoryPath,
            options
    ...
    public async rename(
        ...
        const result = await zowe.Utilities.renameUSSFile(
            this.getSession(),
            currentUssPath,
            newUssPath
        );
...
export class ZosmfMvsApi extends ZosmfApiCommon implements ZoweExplorerApi.IMvs {
    ...
    public async putContents(
        ...
        return zowe.Upload.pathToDataSet(
            this.getSession(),
            inputFilePath,
            dataSetName,
            options
    ...
    public async createDataSetMember(
        ...
        return zowe.Upload.bufferToDataSet(
            this.getSession(),
            Buffer.from(""),
            dataSetName,
            options
    ...
    public async renameDataSetMember(
        ...
        return zowe.Rename.dataSetMember(
            this.getSession(),
            currentMemberName,
            newMemberName,
            afterMemberName
...
export class ZosmfJesApi extends ZosmfApiCommon implements ZoweExplorerApi.IJes {
    public getJobsByOwnerAndPrefix(
        owner: string,
        prefix: string
    ): Promise<zowe.IJob[]> {
        return zowe.GetJobs.getJobsByOwnerAndPrefix(
            this.getSession(),
            owner,
            prefix
        );
    ...
    public downloadSpoolContent(
        parms: zowe.IDownloadAllSpoolContentParms
    ): Promise<void> {
        return zowe.DownloadJobs.downloadAllSpoolContentCommon(
            this.getSession(),
            parms
        );
    }

    public getSpoolContentById(
        jobname: string,
        jobid: string,
        spoolId: number
    ): Promise<string> {
        return zowe.GetJobs.getSpoolContentById(
            this.getSession(),
            jobname,
            jobid,
            spoolId
    ...
    public submitJcl(
        ...
        return zowe.SubmitJobs.submitJcl(
            this.getSession(),
            jcl,
            internalReaderRecfm,
            internalReaderLrecl
