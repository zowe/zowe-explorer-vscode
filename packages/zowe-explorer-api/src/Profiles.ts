...
export class Profiles {
  ...
  public async promptCredentials(sessName, rePrompt?: boolean) {
    ...
    if (newUser === undefined) {
      vscode.window.showInformationMessage(localize("promptCredentials.undefined.username", "Operation Cancelled"));
      await this.refresh();
      return undefined;
    } else {
    ...
    if (newPass === undefined) {
      vscode.window.showInformationMessage(localize("promptCredentials.undefined.password", "Operation Cancelled"));
      await this.refresh();
      return undefined;
    } else {
      try {
  ...
  private async deletePrompt(deletedProfile: IProfileLoaded) {
    ...
    try {
      ...
      this.log.error(
        localize("deleteProfile.delete.log.error", "Error encountered when deleting profile! ") + JSON.stringify(error)
      );
