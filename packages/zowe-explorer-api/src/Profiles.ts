...
export class Profiles {
    ...
    public async checkCurrentProfile(theProfile: IProfileLoaded) {
        ...
        if (!theProfile.profile.user || !theProfile.profile.password) {
            try {
                const values = await Profiles.getInstance().promptCredentials(
                    theProfile.name
                );
                if (values !== undefined) {
            ...
            if (
                this.usrNme !== undefined &&
                this.passWrd !== undefined &&
                this.baseEncd !== undefined
            ) {
    ...
    public loadNamedProfile(name: string, type?: string): IProfileLoaded {
        ...
        throw new Error(
            localize(
                "loadNamedProfile.error.profileName",
    ...
    public async refresh(): Promise<void> {
        ...
        for (const type of ZoweExplorerApiRegister.getInstance().registeredApiTypes()) {
            ...
            const profilesForType = (await profileManager.loadAll()).filter(
                (profile) => {
    ...
    public async createZoweSession(zoweFileProvider: IZoweTree<IZoweTreeNode>) {
        ...
        const items: vscode.QuickPickItem[] = profileNamesList.map(
            (element) => new FilterItem(element)
        );
        ...
        if (chosenProfile === "") {
            ...
            this.log.debug(
                localize(
                    "addSession.log.debug.createNewProfile",
                    "User created a new profile"
                )
            );
            try {
            ...
            this.log.debug(
                localize(
                    "createZoweSession.log.debug.selectProfile",
            ...
            this.log.debug(
                localize(
                    "createZoweSession.log.debug.cancelledSelection",
                    "User cancelled profile selection"
                )
            );
        }
    }
    ...
    public async editSession(
            ...
            switch (value) {
                ...
                case "user":
                    ...
                    if (updUser === undefined) {
                        vscode.window.showInformationMessage(
                            localize(
                                "createNewConnection.undefined.username",
                                "Operation Cancelled"
                            )
                        );
                        return undefined;
                ...
                default:
                    ...
                    switch (response) {
                        case "number":
                            options = await this.optionsValue(
                                value,
                                schema,
                                editSession[value]
                            );
                            const updValue = await vscode.window.showInputBox(
                                options
                            );
                            if (!Number.isNaN(Number(updValue))) {
                                ...
                                if (
                                    schema[value].optionDefinition.hasOwnProperty(
                                        "defaultValue"
                                    )
                                ) {
                                    updSchemaValues[value] =
                                        schema[value].optionDefinition.defaultValue;
                                } else {
                                    break;
                                }
                            }
                        ...
                        case "boolean":
                            ...
                            if (updIsTrue === undefined) {
                                vscode.window.showInformationMessage(
                                    localize(
                                        "createNewConnection.booleanValue",
                                        "Operation Cancelled"
                                    )
                                );
                                return undefined;
                        ...
                        default:
                            ...
                            const updDefValue = await vscode.window.showInputBox(
                                options
                            );
                            if (updDefValue === "") {
                                break;
        ...
        try {
            const updSession = await zowe.ZosmfSession.createBasicZosmfSession(
                updSchemaValues
            );
            updSchemaValues.base64EncodedAuth =
    ...
    public async getProfileType(): Promise<string> {
        ...
        if (typeOptions.length === 1 && typeOptions[0] === "zosmf") {
            ...
            const quickPickTypeOptions: vscode.QuickPickOptions = {
                placeHolder: localize(
                    "createNewConnection.option.prompt.type.placeholder",
                    "Profile Type"
                ),
                ignoreFocusOut: true,
                canPickMany: false,
            };
            profileType = await vscode.window.showQuickPick(
                typeOptions,
                quickPickTypeOptions
            );
        }
        return profileType;
    }
    ...
    public async createNewConnection(
        ...
        if (newProfileName === undefined || newProfileName === "") {
            vscode.window.showInformationMessage(
                localize(
                    "createNewConnection.profileName",
                    "Profile name was not supplied. Operation Cancelled"
                )
            );
            return undefined;
        ...
        if (profileType === undefined) {
            vscode.window.showInformationMessage(
                localize(
                    "createNewConnection.profileType",
            ...
            switch (value) {
                ...
                default:
                    ...
                    switch (response) {
                        ...
                        default:
                            options = await this.optionsValue(value, schema);
                            const defValue = await vscode.window.showInputBox(
                                options
                            );
                            if (defValue === "") {
                                break;
        ...
        try {
            ...
            vscode.window.showInformationMessage(
                "Profile " + newProfileName + " was created."
            );
    ...
    public async promptCredentials(sessName, rePrompt?: boolean) {
        ...
        if (newUser === undefined) {
            vscode.window.showInformationMessage(
                localize(
                    "promptCredentials.undefined.username",
                    "Operation Cancelled"
                )
            );
            await this.refresh();
        ...
        if (newPass === undefined) {
            vscode.window.showInformationMessage(
                localize(
                    "promptCredentials.undefined.password",
                    "Operation Cancelled"
                )
            );
            await this.refresh();
            ...
            try {
                const updSession = await ZoweExplorerApiRegister.getMvsApi(
                    loadProfile
                ).getSession();
                if (rePrompt) {
                    const saveButton = localize(
                        "promptCredentials.saveCredentials.button",
                        "Save Credentials"
                    );
                    const doNotSaveButton = localize(
                        "promptCredentials.doNotSave.button",
                        "Do Not Save"
                    );
    ...
    public async getDeleteProfile() {
        ...
        if (!profileNamesList.length) {
            vscode.window.showInformationMessage(
                localize("deleteProfile.noProfilesLoaded", "No profiles available")
            );
            return;
        }
        ...
        if (sesName === undefined) {
            vscode.window.showInformationMessage(
                localize(
                    "deleteProfile.undefined.profilename",
                    "Operation Cancelled"
                )
            );
            return;
    ...
    public async deleteProfile(
        ...
        if (!deleteSuccess) {
            vscode.window.showInformationMessage(
                localize("deleteProfile.noSelected", "Operation Cancelled")
            );
            return;
        }
        ...
        fileHistory
            ...
            .filter(
                (ds) =>
                    ds.substring(1, ds.indexOf("]")).trim() ===
        ...
        datasetTree.mFavorites.forEach((favNode) => {
            const findNode = favNode.label
                .substring(1, favNode.label.indexOf("]"))
                .trim();
        ...
        fileHistoryUSS
            ...
            .filter(
                (uss) =>
                    uss.substring(1, uss.indexOf("]")).trim() ===
                    deleteLabel.toUpperCase()
            )
            .forEach((uss) => {
        ...
        await vscode.workspace
            .getConfiguration()
            .update(this.dsSchema, dsSetting, vscode.ConfigurationTarget.Global);
        ...
        await vscode.workspace
            .getConfiguration()
            .update(this.ussSchema, ussSetting, vscode.ConfigurationTarget.Global);
    ...
    public async validateProfiles(theProfile: IProfileLoaded) {
        ...
        let profileStatus;
        const getSessStatus = await ZoweExplorerApiRegister.getInstance().getCommonApi(
            theProfile
        );
                ...
                if (getSessStatus.getStatus) {
                    profileStatus = await vscode.window.withProgress(
                        ...
                        (progress, token) => {
                            ...
                            return getSessStatus.getStatus(
                                theProfile,
                                theProfile.type
                            );
                        }
    ...
    private async deletePrompt(deletedProfile: IProfileLoaded) {
        ...
        this.log.debug(
            localize("deleteProfile.log.debug", "Deleting profile ") + profileName
        );
        ...
        if (
            ...
            this.log.debug(
                localize(
                    "deleteProfile.showQuickPick.log.debug",
                    "User picked Cancel. Cancelling delete of profile"
                )
            );
            return;
        }
        ...
        try {
            ...
            this.log.error(
                localize(
                    "deleteProfile.delete.log.error",
                    "Error encountered when deleting profile! "
    ...
    private async deleteProfileOnDisk(ProfileInfo) {
        ...
        try {
            zosmfProfile = await (
                await this.getCliProfileManager(ProfileInfo.type)
            ).delete({
                profile: ProfileInfo,
    ...
    private async urlInfo(input?) {
        ...
        urlInputBox.placeholder = localize(
            "createNewConnection.option.prompt.url.placeholder",
            "https://url:port"
        );
    ...
    private async portInfo(input: string, schema: {}) {
        ...
        if (schema[input].optionDefinition.hasOwnProperty("defaultValue")) {
            ...
            options = {
                placeHolder: localize(
                    "createNewConnection.option.prompt.port.placeholder",
                    "Port Number"
                ),
                prompt: schema[input].optionDefinition.description.toString(),
            };
        ...
        if (
            port === 0 &&
            schema[input].optionDefinition.hasOwnProperty("defaultValue")
        ) {
    ...
    private async userInfo(input?) {
        ...
        InputBoxOptions = {
            placeHolder: localize(
                "createNewConnection.option.prompt.username.placeholder",
                "Optional: User Name"
            ),
            prompt: localize(
        ...
        if (userName === undefined) {
            vscode.window.showInformationMessage(
                localize(
                    "createNewConnection.undefined.passWord",
                    "Operation Cancelled"
                )
            );
            return undefined;
    ...
    private async passwordInfo(input?) {
        ...
        InputBoxOptions = {
            placeHolder: localize(
                "createNewConnection.option.prompt.password.placeholder",
                "Optional: Password"
            ),
            prompt: localize(
    ...
    private async boolInfo(input: string, schema: {}) {
        let isTrue: boolean;
        const description: string = schema[
            input
        ].optionDefinition.description.toString();
        ...
        const chosenValue = await vscode.window.showQuickPick(
            selectBoolean,
            quickPickBooleanOptions
        );
    ...
    private async updateProfile(ProfileInfo, rePrompt?: boolean) {
        ...
        try {
            (await this.getCliProfileManager(this.loadedProfile.type)).update(
                updateParms
            );
        } catch (error) {
            vscode.window.showErrorMessage(error.message);
