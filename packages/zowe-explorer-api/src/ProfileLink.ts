      ...
      if (chosenName) {
        try {
          ...
          vscode.window.showErrorMessage(
            localize("profileLink.unableToSave", "Unable to save profile association. ") + err.message
          );
        }
      }
    }
          ...
          for (const element of Object.keys(links)) {
            if (element === type) {
              try {
                ...
                throw new Error(
                  localize("profileLink.missingProfile", "Attempted to load a missing profile.") + " + " + err.message
                );
              }
            }
          }
